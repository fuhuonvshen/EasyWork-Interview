...
EasyWork 注重**隐私优先**——音频录制与语音转写默认在本地运行（Whisper / SenseVoice），数据不出设备；同时支持接入云端大模型（在线 LLM / 在线语音识别）获得更强大的 AI 能力，投递公司库支持云端共享。
...
- **通用助手**：岗位调研、面经问答、薪资谈判
- **面试题库**：AI 复盘时自动提取面试官问过的问题，你确认后归档，下次面试前针对性复习
- 长期记忆（求职档案：目标岗位、偏好、决策）
